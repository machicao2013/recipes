    ...
    do {
        ...
        for (i=0; i <= max_sd  &&  desc_ready > 0; ++i) {
            ...
            if (FD_ISSET(i, &working_set)) {
                // A descriptor was found that was readable - one less has to be looked for.  This is being done
                // so that we can stop looking at the working set once we have found all of the descriptors that
                // were ready.
                desc_ready -= 1;
                ...
                if (i == listen_sd) {
                    ...
                    do {
                        // Accept each incoming connection.  If accept fails with EWOULDBLOCK, then we
                        // have accepted all of them.  Any other failure on accept will cause us to end the
                        // server.
                    ...
                    // If the close_conn flag was turned on, we need to clean up this active connection.  This
                    // clean up process includes removing the descriptor from the master set and determining the new maximum
                    // descriptor value based on the bits that are still turned on in the master set.
                    if (close_conn) {
                        close(i);/*{{{*/
                        FD_CLR(i, &master_set);
                        if (i == max_sd)
